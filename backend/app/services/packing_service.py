...
async def get_packing_items(trip_id: int, user: User):
    await get_trip(trip_id, user)
    return await PackingItem.filter(trip_id=trip_id).all()

async def create_packing_item(trip_id: int, user: User, item_data: PackingItemCreate):
    await get_trip(trip_id, user)
    return await PackingItem.create(trip_id=trip_id, **item_data.model_dump())

async def update_packing_item(trip_id: int, item_id: int, user: User, item_data: PackingItemUpdate):
    await get_trip(trip_id, user)
...
async def delete_packing_item(trip_id: int, item_id: int, user: User):
    await get_trip(trip_id, user)
    item = await PackingItem.get_or_none(id=item_id, trip_id=trip_id)
    if not item:
        raise HTTPException(status_code=404, detail="Packing item not found")
...
async def reset_packing_items(trip_id: int, user: User):
    await get_trip(trip_id, user)
