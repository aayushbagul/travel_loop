...
export default function UserTripListing() {
  ...
  const TripCard = ({ trip }) => (
    <div className="bg-white rounded-2xl p-5 sm:p-6 shadow-sm border border-white/10 hover:shadow-md hover:border-indigo-400 transition-all group">
      {/* Title + dates row */}
      <div
      ...
      >
        <div className="flex-1 min-w-0">
          <h3 className="text-lg font-bold text-white group-hover:text-indigo-400 transition-colors">{trip.title}</h3>
          <p className="text-slate-300 text-sm mt-1">{formatDates(trip)}</p>
          {trip.description && <p className="text-slate-400 text-sm mt-1 line-clamp-2">{trip.description}</p>}
        </div>
        <svg className="w-5 h-5 text-slate-400 group-hover:text-indigo-400 transition shrink-0 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex flex-wrap gap-2 pt-3 border-t border-white/10">
        <button
          onClick={() => navigate(`/itinerary-view/${trip.id}`)}
          className="flex-1 min-w-[100px] px-4 py-2 bg-indigo-600/20 text-indigo-300 font-semibold rounded-xl hover:bg-indigo-600 hover:text-white transition text-sm"
        >
          View Itinerary
        </button>
        <button
          onClick={() => navigate(`/build-itinerary/${trip.id}`)}
          className="flex-1 min-w-[80px] px-4 py-2 bg-white/10 text-white font-semibold rounded-xl hover:bg-white/20 transition text-sm"
        >
          ✏️ Build
        </button>
        <button
          onClick={() => navigate(`/notes/${trip.id}`)}
          className="px-3 py-2 bg-white/10 text-white rounded-xl hover:bg-indigo-500/20 hover:text-indigo-300 transition text-sm"
          title="Notes"
        >📝</button>
        <button
        ...
        <button
          ...
          className="px-3 py-2 bg-red-500/10 text-red-400 rounded-xl hover:bg-red-500/20 transition text-sm"
          title="Delete"
        >🗑️</button>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-transparent font-sans pb-24">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        {/* Search and Filters */}
        <div className="flex flex-col md:flex-row gap-4 mb-10 items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-white/10">
        ...
        {loading && (
          <div className="text-center py-20">
            ...
            <p className="text-slate-300 mt-4 font-medium">Loading your trips...</p>
          </div>
        )}

        {error && <div className="p-4 bg-red-500/10 text-red-400 rounded-xl font-medium border border-red-500/20 mb-6">{error}</div>}

        {!loading && trips.length === 0 && (
          <div className="text-center py-20 bg-white rounded-3xl border border-white/10 shadow-sm">
            <p className="text-5xl mb-4">🌍</p>
            <h3 className="text-xl font-bold text-white mb-2">No trips yet</h3>
            <p className="text-slate-300 mb-6">Start planning your next adventure!</p>
            <button
              onClick={() => navigate('/create-trip')}
